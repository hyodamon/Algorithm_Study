...
이 문제도 그래프와 상하좌우를 이동해야한다는 조건이 있으므로, BFS 또는 DFS를 사용해야하는 문제라 판단했다.

그리고 배추흰지렁이는 인접한 배추들로만 이동할 수 있기 때문에, 인접한 배추들의 그룹의 수를 구하면 되는 문제이다.

먼저, 농장을 나타내는 2차원 배열 `farm`과, 그 농장을 방문한 것을 확인할 수 있는 2차원 배열 `visited`를 선언해준다.

`farm`을 (0, 0)부터 하나씩 확인하면서, 배추가 있고 이미 방문한 곳인지(farm[r][c] == 1 and visited[r][c] == 0) 확인한다.

위 조건에 맞다면, 그 자리에 배추흰지렁이를 놓아주어야 하기 때문에 정답에 1씩 더해준다.

그리고 dfs를 통해 배추가 있는 자리에 상하좌우를 확인하면서 인접한 배추들을 다 방문처리(visited[nr][nc] = 1) 해준다.

위 과정을 반복하면, 배추가 모여있는 구역의 개수를 구해 최소의 배추흰지렁이 마리 수를 구할 수 있다.
